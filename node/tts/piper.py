...
class PiperTTS:
    ...
    def validate(self) -> None:
        """Check that the configured Piper model exists."""

        if not self.model_path.is_file():
            raise PiperError(
                f"Δεν βρέθηκε το μοντέλο Piper: {self.model_path}"
            )
        ...
        if not config_path.is_file():
            logger.warning(
                "Δεν βρέθηκε το συνοδευτικό αρχείο μοντέλου: {}",
                config_path,
            )
    ...
    async def _synthesize(self, text: str, output_path: Path) -> None:
        ...
        if process.returncode != 0:
            ...
            raise PiperError(
                f"Η δημιουργία φωνής απέτυχε: {error or process.returncode}"
            )
    ...
    async def _play(self, audio_path: Path) -> None:
        ...
        if process.returncode != 0:
            ...
            raise PiperError(
                f"Η αναπαραγωγή ήχου απέτυχε: {error or process.returncode}"
            )